...
public class SimulationEngine {
    ...
    public static void createVM(int userId) {
        ...
        for (int i = init; i < end; i++) {
            if (isRR) {
                vm = new HzVm(i, userId, mips, pesNumber, ram, bw, (int) ((Math.random() * size) + 1), vmm, new CloudletSchedulerTimeShared());
            } else {
                vm = new HzVm(i, userId, mips, pesNumber, ram, bw, (int) ((Math.random() * size) + 1), vmm, new CloudletSchedulerSpaceShared());
            }
